...
for i = 0:(1/columns):1
    ...
    for j = 0:(1/rows):1     
        ...
        if transform == 0
            ...
            svs_2 = [svs_2, x1+((1/columns)*width)];
            sus_2 = [sus_2, y1];
            
            plot([x1 x1+((1/columns)*width)],[y1 y1],'k');
            hold on;
        else
            tspan = 0:timestep:timestep*2;
            [t1,t_r1] = ode45(@rybak, tspan, [x1 y1]);
            %t_r1 = rybak([x1,y1],timestep);
            [t1,t_r2] = ode45(@rybak, tspan, [x1+((1/columns)*width) y1]);
