...
% % 3D using m_inf
% tspan = 0:0.01:100.0;
% [t,s] = ode23s(@HHv_rybak, tspan, [v_0 h_0 n_0]);
% vs = s(:,1);
% plot(t,vs,'b');
% hold on;
...
%     [t,s,te,ye,ie] = ode23s(@HHv_rybak_1D, tspan, [-70.8], reset);
%     vs = s(:,1);
%     plot(t,vs,'r');
%     hold on;
%     tspan = te+1.9:0.01:100.0;
% end
...
v_0 = -48.62;
[t,s] = ode23s(@HHv_rybak_1D, tspan, [v_0]);
...
fprintf(outId, '<Quadrilateral><vline>%f %f %f %f</vline><wline>%f %f %f %f</wline></Quadrilateral>\n',-56.069, -56.069, -56.07, -56.07, 0.0, 0.05, 0.05, 0.0);
fprintf(outId, '<Quadrilateral><vline>%f %f %f %f</vline><wline>%f %f %f %f</wline></Quadrilateral>\n',-48.615, -48.615, -48.605, -48.605, 0.0, 0.05, 0.05, 0.0);
fprintf(outId, '</Stationary>\n');
fclose(outId);
...
function vec = HHv_rybak(t,vs)
    ...
    h_na_inf = ((1 + (exp((v + 55)/7)))^(-1));
    tau_h_na = 30 / (exp((v+50)/15) + exp(-(v+50)/16));
    h_na_prime = (h_na_inf - h_na) / tau_h_na;
    
    m_na_inf = ((1 + exp(-(v+35)/7.8))^-1);
    
    E_na = 55;
    I_na = g_na * (m_na_inf^3) * (h_na) * (v - E_na);
    
    g_k = 100;
    
    m_k_inf = ((1 + (exp(-(v + 28)/15)))^(-1));
    tau_m_k = 7 / (exp((v+40)/40) + exp(-(v + 40)/50));
    m_k_prime = (m_k_inf - m_k) / tau_m_k;
    ...
    vec = [v_prime;h_na_prime;m_k_prime];
end

function vec = HHv_rybak_1D(t,vs)
    global I;
    ...
    h_na_inf = max(h_na_inf,0.2);
    
    m_na_inf = ((1 + exp(-(v+35)/7.8))^-1);
    
    E_na = 55;
    I_na = g_na * (m_na_inf^3) * (h_na_inf) * (v - E_na);
