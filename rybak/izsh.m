...
stripend = nullcline_strip_length-(1*strip_cell_strip_width);

i = 5+(1*strip_cell_strip_width);
strip_cell_strip_width = 3;
while (i < stripend)
    if (i+strip_cell_strip_width > length(left_curve(:,1))) 
        i = length(left_curve(:,1))-1-strip_cell_strip_width;
    end
    ...
    strip_cell_strip_width = 3 + ceil(((i / stripend) * 50))
...
stripend = ((small_strip_start * strip_cell_strip_width)-small_strip_width)+5;
i = 5+(small_strip_width);
small_strip_width = 3;
while (i<stripend)
    if (i+small_strip_width > length(right_curve(:,1))) 
        i = length(right_curve(:,1))-1-small_strip_width;
    end
...
stripend = 5+nullcline_strip_length-strip_cell_strip_width;
small_strip_width = 7;
strip_cell_strip_width = 7;

while i < stripend
    if (i+strip_cell_strip_width > length(right_curve(:,1))) 
        i = length(right_curve(:,1))-1-strip_cell_strip_width;
    end
    ...
    strip_cell_strip_width = 7 + ceil(((i / stripend) * 35))
...
for h0 = h_init:0.01:0.7
    ...
    [t1,s1] = ode23s(@izshikevich_backward, tspan, [v0 h0]);
    [t2,s2] = ode23s(@izshikevich_backward, tspan, [v0 h0 + 0.01]);
    ...
    jmp = -0.05 * eff * (((((47+h0)/2.0) + ((47+h0+0.01)/2.0)) / 2.0) + 10);
    fprintf(jmpId, '%i,%i %f 0\n', strip, cell, jmp);

    jmpInhib = 0.05 * effInhib * (((((47+h0)/2.0) + ((47+h0+0.01)/2.0)) / 2.0) + 70);
    fprintf(jmpInhibId, '%i,%i %f 0\n', strip, cell, jmpInhib);

    jmpBig = 0.05 * effBig * (((((47+h0)/2.0) + ((47+h0+0.01)/2.0)) / 2.0) + 70);
    fprintf(jmpBigId, '%i,%i %f 0\n', strip, cell, jmpBig);
    ...
    fprintf(outId, formatSpec, svs_1);
    fprintf(outId, formatSpec, -40.0);
    fprintf(outId, '\n');
    fprintf(outId, formatSpec, sus_1);
    fprintf(outId, formatSpec, h0);
    fprintf(outId, '\n');
    ...
    fprintf(outId, '\n');
    fprintf(outId, formatSpec, sus_2);
    fprintf(outId, formatSpec, h0 + 0.01);
    fprintf(outId, '\n');
    fprintf(outId, 'closed\n');
