...
% fprintf(outId, '<Quadrilateral><vline>%f %f %f %f</vline><wline>%f %f %f %f</wline></Quadrilateral>\n',-63.855, -63.855, -63.845, -63.845, 0.0, 0.05, 0.05, 0.0);
% fprintf(outId, '<Quadrilateral><vline>%f %f %f %f</vline><wline>%f %f %f %f</wline></Quadrilateral>\n',-57.25, -57.25, -57.15, -57.15, 0.0, 0.05, 0.05, 0.0);
% fprintf(outId, '</Stationary>\n');
% fclose(outId);
...
function vec = HHv_exp(t,vs)
    ...
    v_prime = (1.0/tau) * ((g_l * delta_t * exp((v - v_th)/delta_t)) - (g_l * (v - v_l)) + I);
